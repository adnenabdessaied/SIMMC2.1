...
from src.models.vst_transformer_based import VstTransformer
from src.models.dst_transformer_based import DstTransformer
from src.models.combiner import CombinerOptionA
...
class StateTrackerModel(TransformerModel):
    def __init__(self, config, output_path=None):
        ...
        if self.ext_config['use_v_state']:
            self.video_state_tracker = VstTransformer(
                self.model_input_dim,
                self.config['dropout_p'],
                self.ext_config
            )
        if self.ext_config['use_d_state']:
            self.dial_state_tracker = DstTransformer(
                self.model_input_dim,
                self.config['dropout_p'],
                self.ext_config
            )
